...
/*    *//**
    ...
    *//**
     ...
     * @return
     */
    public String generateToken(UserBaseInformationVO token) {
        ...
        String userNo=token.getNo();
        if(StringUtils.isNotBlank(userNo)) {
        	 claims.put(CLAIM_KEY_USERNO, userNo);	
        }
    ...
    public String getUserNoFromToken(String token) {
        ...
        try {
            Claims claims = getClaimsFromToken(token);
            userNo =(String)claims.get(CLAIM_KEY_USERNO);
        } catch (Exception e) {
        	 log.error(e.getMessage());  
        }
        return userNo;
     ...
     * @return
     */
    public UserBaseInformationVO getUserInfoFromToken(String token) {
    	UserBaseInformationVO userInfo = null;
        try {
            Claims claims = getClaimsFromToken(token);
            userInfo =JsonUtil.jsonToPojo((String)claims.get(CLAIM_KEY_USERINFO), UserBaseInformationVO.class);
        } catch (Exception e) {
        	 log.error(e.getMessage());  
        }
        return userInfo;
  ...
  /*  public UserBaseInformationDTO getTokenInfo(HttpServletRequest request) {
    ...
    public static void main(String args[]) {
    	...
    	UserBaseInformationVO input=new UserBaseInformationVO();
    	input.setId(Long.valueOf("1111111"));
    	...
    	System.out.println(test.getUserNoFromToken(token));
    	System.out.println(test.getUserTypeFromToken(token));
